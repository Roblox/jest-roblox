...
return function()
	...
	local function get(object, lensPath)
		return Array.reduce(lensPath, function(acc, key)
			return acc[key]
		end, object) --[[ ROBLOX CHECK: check if 'lensPath' is an Array ]]
	end
	local function getGlobalTestMocks()
		local globals: any = {
			describe = jest.fn(),
			describeFOCUS = jest.fn(),
			describeSKIP = jest.fn(),
			fdescribe = jest.fn(),
			fit = jest.fn(),
			it = jest.fn(),
			itFOCUS = jest.fn(),
	...
	-- ROBLOX TODO: skipping whole file while feature is not implemented
	describeSKIP("jest-each", function()
		Array.forEach({
			-- { "test" },
			-- { "test", "concurrent" },
			...
			describe(
				...
				function()
					it("throws error when there are additional words in first column heading", function()
						...
						jestExpect(testCallBack).not_.toHaveBeenCalled()
					end)
          ...
          a    | b    | expected
					...
					it("calls global with given timeout", function()
						...
						local globalMock = get(globalTestMocks, keyPath)
						jestExpect(globalMock).toHaveBeenCalledWith("some test", expect:any("function"), 10000)
					end)
